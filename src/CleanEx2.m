...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Note: This only works with the EEG dataset from Fischi
%
...
% sequence were a volume repetition time of TR=2s with 25 axial slices per
% volume and a slice time of 80ms. The sequence does not have an
...
%   11-18: Emg10, Cz, Pz, HR, ECG, LatFrontalis, CorrSupcilii, OrbiOcculi,
%   19-22: ZygomMajor, Masseter, VEOG, HEOG
%
% 2011-06-26: removed pre-fMRI peaks:
%   for i=1:22
%     EEG.data(i,[12230:14000]) = EEG.data(i,12229);
%   end
%
% Range:
% raw unipolar EMG:
%   min(min(EEG_Fischi.data([1,2,3,4,6,7,8,9,10,11],:))) = -2.6344e+05
%   max(max(EEG_Fischi.data([1,2,3,4,6,7,8,9,10,11],:))) = 2.8640e+05
% bidirectional EMG
%   min(min(EEG_Fischi.data([16:20],:))) = -2.5141e+05
%   max(max(EEG_Fischi.data([16:20],:))) = 3.4221e+05
% EEG
%   min(min(EEG_Fischi.data([5,12,13],:))) = -1.5600e+05
%   max(max(EEG_Fischi.data([5,12,13],:))) = 5.0384e+05
% EOG
%   min(min(EEG_Fischi.data([21,22],:))) = -2.1304e+05
%   max(max(EEG_Fischi.data([21,22],:))) = 5.0894e+05
%
% Note: Most data is EMG data, for which a 10Hz HP can be used.
%
function EEGout = CleanEx2(EEGin)
  ...
  E.AvgMatrixUpdaterData.SearchHalfWindow = 10*E.HalfWindow;
  % = 300
