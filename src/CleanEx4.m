...
% EEGLAB plugin and performs an artifact removal algorithm modeled after
% Allen et al. (2000).
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%   each, 840 in total.
%
function EEGout = CleanEx4(EEGin,DoLowPass,DoANC,DoUpsample1)
  ...
  if nargin < 4
    DoUpsample1 = false;   % should be 0.5 (i.e. set to false) to model Allen
  end
  ...
  %E.PreFilterGaussHPFrequency = 1.0;   % not necessary here
  ...
  %E.SSAHPFrequency = 300;   % not necessary here
  ...
  % (default).
  %E.OBSNumPCs = 'auto';   % not necessary here
  % Channels not to perform OBS on
  %E.OBSExcludeChannels=[31 32];   % not necessary here
  % High pass cutoff frequency applied before PCA
  %E.OBSHPFrequency = 300;   % not necessary here
  ...
  %   volume triggers.
  %E.FindMissingTriggers(40,21);   % not necessary here
  ...
  % conditions.
  %
  %E.CheckData;   % not necessary here
